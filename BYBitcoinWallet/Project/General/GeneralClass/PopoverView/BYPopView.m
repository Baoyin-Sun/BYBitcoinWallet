...
static float const kPopoverViewMargin = 0.f;         ///< 边距
...
@interface BYPopView () <UITableViewDelegate, UITableViewDataSource>
...
#pragma mark - Data
@property (nonatomic, copy) NSArray<BYPopViewAction *> *actions;
...
- (void)setShowShade:(BOOL)showShade {
    ...
    _shadeView.backgroundColor = _showShade ? [UIColor colorWithWhite:0.f alpha:0.6f] : [UIColor clearColor];
    if (_borderLayer) {
...
- (void)setStyle:(PopoverViewStyle)style {
    _style = style;
    _tableView.separatorColor = [BYPopViewCell bottomLineColorForStyle:_style];
    if (_style == PopoverViewStyleDefault) {
        self.backgroundColor = [UIColor whiteColor];
    } else {
        self.backgroundColor = [UIColor by_colorWithHexString:@"F000000" Alpha:0.9];
    }
}
...
- (void)showToPoint:(CGPoint)toPoint {
    ...
    CGFloat currentW = _mode == PopoverViewModeDefault? [self calculateMaxWidth] : CGRectGetWidth(_keyWindow.bounds); // 宽度通过计算获取最大值
    CGFloat currentH = self.actions.count * kPopoverViewCellHeight + kPopoverViewArrowHeight;
...
- (CGFloat)calculateMaxWidth {
    ...
    UIFont *titleFont = [BYPopViewCell titleFont];
    for (BYPopViewAction *action in _actions) {
        imageWidth = 0.f;
        titleLeftEdge = 0.f;
        ...
        if (action.iconAndtitle) {
            titleWidth = [action.iconAndtitle size].width;
        } else {
            titleWidth = [action.title sizeWithAttributes:@{NSFontAttributeName : titleFont}].width + by_autoResize(50);
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    BYPopViewCell *cell = [tableView dequeueReusableCellWithIdentifier:kPopoverCellIdentifier];
    if (!cell) {
        cell = [[BYPopViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kPopoverCellIdentifier];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [UIView animateWithDuration:0.25f animations:^{
        ...
        BYPopViewAction *action = self->_actions[indexPath.row];
